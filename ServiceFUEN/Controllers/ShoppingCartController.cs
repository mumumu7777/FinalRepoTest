...
namespace ServiceFUEN.Controllers
{
	...
	public class ShoppingCartController : ControllerBase
	{
		...
		public async Task<IActionResult> DeleteCartItem([Bind(nameof(CartProduct))] CartProduct item, int memberid)
		{
			...
			//找到該購物車商品
			var CartItem = _context.ShoppingCarts.Where(x=>x.ProductId ==item.Id).FirstOrDefault();
			var memberselect = _context.ShoppingCarts.Where(x=>x.MemberId== memberid).FirstOrDefault();
			if (memberselect.MemberId == memberid)
			{
